...
$theme = wp_get_theme();
define('RUBBERSOUL_VERSION', $theme -> get('Version'));
define ('RUBBERSOUL_AUTHOR_URI', $theme -> get('AuthorURI'));
...
function rubbersoul_scripts_styles() {
	...
	// Toggle search
	wp_enqueue_script('rsoul-toogle-search', get_template_directory_uri() .'/js/rsoul-toggle-search.js', array('jquery'), RUBBERSOUL_VERSION, true);
}
...
if ( ! function_exists( 'rubbersoul_content_nav' ) ) :
/**
 * Displays navigation to next/previous pages when applicable.
 *
 * @since RubberSoul 1.0
 */
function rubbersoul_content_nav( $html_id ) {
	global $wp_query;
	...
	if ( $wp_query->max_num_pages > 1 ) : ?>
		<nav id="<?php echo $html_id; ?>" class="navigation" role="navigation">
			<h3 class="assistive-text"><?php _e( 'Post navigation', 'rubbersoul' ); ?></h3>
			<div class="wrapper-navigation-below">
			<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'rubbersoul' ) ); ?></div>
			<div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'rubbersoul' ) ); ?></div>
			</div>
...
if ( ! function_exists( 'rubbersoul_entry_meta' ) ) :
/**
 * Set up post entry meta.
 *
 * Prints HTML with meta information for current post: categories, tags, permalink, author, and date.
 *
 * Create your own rubbersoul_entry_meta() to override in a child theme.
 *
 * @since RubberSoul 1.0
 */
function rubbersoul_entry_meta() {
	// Translators: used between list items, there is a space after the comma.
	$categories_list = get_the_category_list( __( ', ', 'rubbersoul' ) );

	// Translators: used between list items, there is a space after the comma.
	$tag_list = get_the_tag_list( '', __( ', ', 'rubbersoul' ) );
	...
	$author = sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
		...
		esc_attr( sprintf( __( 'View all posts by %s', 'rubbersoul' ), get_the_author() ) ),
		get_the_author()
	);

	// Translators: 1 is category, 2 is tag, 3 is the date and 4 is the author's name.
	if ( $tag_list ) {
		$utility_text = __( 'This entry was posted in %1$s and tagged %2$s on %3$s<span class="by-author"> by %4$s</span>.', 'rubbersoul' );
	} elseif ( $categories_list ) {
		$utility_text = __( 'This entry was posted in %1$s on %3$s<span class="by-author"> by %4$s</span>.', 'rubbersoul' );
	} else {
		$utility_text = __( 'This entry was posted on %3$s<span class="by-author"> by %4$s</span>.', 'rubbersoul' );
	}
